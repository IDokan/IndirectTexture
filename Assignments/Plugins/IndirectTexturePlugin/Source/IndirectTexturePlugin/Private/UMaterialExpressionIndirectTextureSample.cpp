...
int32 UMaterialExpressionIndirectTextureSample::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	int32 Y = Compiler->Div(TileIndex, TilesetTilesColumnNum);
	...
	int32 IndirectUV =
		Compiler->Add(Compiler->AppendVector(Compiler->ComponentMask(TileColumn, 1, 0, 0, 0), Compiler->ComponentMask(TileRow, 0, 1, 0, 0)),
			UVOffset
		);
